...
let UI_CHART_MARGIN_RIGHT = 16;
let UI_CHART_PLOT_TOP = 16;
/*jslint-disable*/
(function () {
    "use strict";
let $ = undefined;
...
SVGElement.prototype = {
    ...
    add: function (parent) {
        ...
        // mark as added
        this.added = true;
// hack-uichart - inline event-handler
        // fire an event for internal hooks
        if (this.getSizeAfterAdd) {
            this.getSizeAfterAdd();
        }
        return this;
    },
    /**
...
// hack-uichart - inline event-handler
        wrapper.getSizeAfterAdd = function () {
        // SVGRenderer
            text.add(wrapper);
            wrapper.attr({
...
Tick.prototype = {
    ...
    addLabel: function () {
        let tick = this,
            ...
            css,
            attr,
            value = defined(categories[pos]) ? categories[pos] : pos,
            label = tick.label,
            tickPositionInfo = tickPositions.info,
            dateTimeLabelFormat;
// hack-uichart - only use category labels
        if (!defined(categories[pos])) {
            return;
        }
...
// hack-uichart - yValueSuffix
        if (!axis.isXAxis && chart.options.yValueSuffix) {
            str += chart.options.yValueSuffix;
        }
        // prepare CSS
...
Axis.prototype = {
    ...
    init: function (chart, userOptions) {
        ...
        axis.tickmarkOffset = (options.categories.length && options.tickmarkPlacement === 'between') ? 0.5 : 0;
        // Major ticks
        axis.ticks = {};
        // Minor ticks
        axis.minorTicks = {};
    ...
    setTickPositions: function (secondPass) {
        ...
        if (!categories.length && !axis.usePercentage && defined(axis.min) && defined(axis.max)) {
        ...
        if (axis.min === axis.max || axis.min === undefined || axis.max === undefined) {
            ...
            axis.tickInterval = pick(
                tickIntervalOption,
// hack-uichart - dont bunch up categories
    ...
    adjustTickAmount: function () {
        let axis = this,
            ...
            tickPositions = axis.tickPositions,
            maxTicks = chart.maxTicks;
        if (maxTicks && maxTicks[xOrY] && !axis.isDatetimeAxis && !axis.categories.length && axis.options.alignTicks !== false) { // only apply to linear scale
...
MouseTracker.prototype = {
    ...
    onmousemove: function (e) {
        ...
        if (tooltip && mouseTracker.options.tooltip.shared && !(hoverSeries && hoverSeries.noSharedTooltip)) {
            ...
            i = seriesList.length;
            for (j = 0; j < i; j++) {
                if (seriesList[j].visible &&
                        seriesList[j].options.enableMouseTracking !== false &&
                        !seriesList[j].noSharedTooltip && seriesList[j].tooltipPoints && seriesList[j].tooltipPoints.length) {
                    point = seriesList[j].tooltipPoints[index];
                    point._dist = mathAbs(index - point[seriesList[j].xAxis.tooltipPosName || 'plotX']);
    ...
    setDOMEvents: function () {
        ...
        container.onmouseleave = mouseTracker.hideTooltipOnMouseLeave;
...
Chart.prototype = {
     ...
     *    configuration
     */
    uichartRedraw: function (animation) {
        let chart = this,
            axes = chart.axisList,
            seriesList = chart.seriesList,
            tracker = chart.tracker,
            isDirtyBox = chart.isDirtyBox, // todo: check if it has actually changed?
            seriesLength = seriesList.length,
            i = seriesLength,
            serie,
            renderer = chart.renderer,
        ...
        each(seriesList, function (serie) {
    ...
    getContainer: function () {
        ...
        chart.renderer = new SVGRenderer(container, chartWidth, chartHeight);
    },
    /**
...
// hack-uichart - draw series in reverse,
// so first series (likely most important) is on top
        Array.from(chart.seriesList).reverse().forEach(function (serie) {
            serie.translate();
    ...
    destroy: function () {
        ...
        i = seriesList.length;
        while (i--) {
            seriesList[i] = seriesList[i].destroy();
        }
        // ==== Destroy chart properties:
        each(['plotBGImage',
                'plotBorder', 'seriesGroup', 'clipRect', 'tracker', 'scroller',
                'tooltip', 'renderer'], function (name) {
            let prop = chart[name];
...
// hack-uichart - yValueSuffix
                if (key === "{point.y}" && chart.options.yValueSuffix) {
                    replacement += chart.options.yValueSuffix;
                }
...
Series.prototype = {
    ...
    processData: function (force) {
        ...
        if (isCartesian && series.sorted && (dataLength > 256)) {
    ...
    translate: function () {
        ...
        for (let i = 0; i < dataLength; i++) {
            ...
            // some API data
            point.category = categories[point.x] !== UNDEFINED ?
        ...
        if (seriesMarkerOptions.enabled || series._hasPointMarkers) {
            ...
            while (i--) {
                ...
                if (enabled && plotY !== UNDEFINED && !isNaN(plotY)) {
                    // shortcuts
                    pointAttr = point.pointAttr[NORMAL_STATE];
                    radius = pointAttr.r;
    ...
    getAttribs: function () {
        ...
        each([HOVER_STATE], function (state) {
    ...
    alignDataLabel: function (point, dataLabel, options, alignTo, isNew) {
        ...
        dataLabel.attr({
            visibility: options.crop === false || chart.isInsidePlot(alignAttr.x, alignAttr.y) || chart.isInsidePlot(plotX, plotY) ?
...
let AreaSeries = extendClass(Series, {
    ...
    closeSegment: function (path, segment) {
        let translatedThreshold = this.yAxis.getThreshold(this.options.threshold);
        path.push(
            L,
...
let ColumnSeries = extendClass(Series, {
    ...
    init: function () {
        ...
        if (chart.hasRendered) {
            each(chart.seriesList, function (otherSeries) {
    ...
    translate: function () {
        ...
        if (options.grouping === false) {
            ...
            each(chart.seriesList, function (otherSeries) {
                ...
                if (otherSeries.type === series.type && otherSeries.visible &&
                    ...
                    if (otherOptions.grouping !== false) { // #1162
        ...
        each(points, function (point) {
            let plotY = mathMin(mathMax(-999, point.plotY), yAxis.len + 999), // Don't draw too far outside plot area (#1303)
            ...
            point.shapeArgs = shapeArgs = chart.renderer.Element.prototype.crisp.call(
                ...
                barY,
                barW,
// hack-uichart - minimum bar-height so datapoint is at least visible
                Math.max(4, barH)
            );
    ...
    alignDataLabel: function (point, dataLabel, options,  alignTo, isNew) {
        let chart = this.chart,
            ...
            inside = (options.inside); // draw it inside the box?
...
/*jslint-enable*/
function uichartRedraw(uichart, animation) {
// this function will redraw <uichart>
    uichart.uichartRedraw(animation);
}
...
window.uichartRedraw = uichartRedraw;
