...
```shell
    (set -ex
    # lgbm
    git grep "v4\.3\.0" || true
    for FILE in .ci.sh sqlmath_base.h
    do
        sed -i -e "s|\<v4\.3\.0\>|v4\.4\.0|g" "$FILE"
    done
    git grep "v4\.3\.0"  || true
    # sqlite
    git grep "3\.42\.0\|3420000" || true
    for FILE in .ci.sh sqlmath_external_sqlite.c
    do
